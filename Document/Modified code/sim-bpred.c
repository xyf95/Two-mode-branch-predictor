...
static int bimod_config[1] =
  { /* new tbl size */2048 };                                             //At least 6 correlated branches are included 

/* 2-level predictor config (<l1size> <l2size> <hist_size> <xor>) */
static int twolev_nelt = 4;
static int twolev_config[4] =
  { /* l1size */1, /* l2size */1024, /* hist */10, /* xor */TRUE};       //GShare algorithm, the length of global history register is 10 bit length
                                                                         //the Pattern History Table have 1024 entries
...
"      PAp     : N, W, M (M == 2^(N+W)), 0\n"
"      gshare  : 1, W, 2^W, 1\n"
"  Predictor `comb' combines a new_mode created by \n  Group 5: Yunfei Xu and Xiaohan Li and a 2-level predictor.\n"
               );
  ...
  opt_reg_int_list(odb, "-bpred:new_mode",
		   "new_mode predictor config (<table size>)",
