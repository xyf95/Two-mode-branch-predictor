	     ...
	     unsigned int retstack_size) /* num entries in ret-addr stack */
{
  ...
  switch (class) {
  case BPredComb:                                      //two-mode predictor created.
    /* bimodal component */
    pred->dirpred.bimod = 
  ...
  unsigned int xor)	    	/* history xor address flag */
{
  ...
  case BPred2Level:
	...
	{
	  pred_dir->config.two.l2table[cnt] = flipflop;              // the counter of 2-level predictor keep the same as before
	  flipflop = 3 - flipflop;
	}
  ...
  case BPred2bit:
    ...
    /* initialize counters to weakly this-or-that */
    flipflop = 3;
    for (cnt = 0; cnt < l1size; cnt++)                          // Change to original SimpleScalar
      {                                                         //even counter initialize to 3, combined with the 2-level counter 1, 
	   	pred_dir->config.bimod.table[cnt] = flipflop;          //  to be 0111, weakly not-taken
        	flipflop = 3 - flipflop;                          //odd counter initialize to 0, combined with the 2-level counter 2,
      }	                                                        // to 1000, weakly taken
    pred_dir->config.bimod.table[0] = -1;                     //the first entry store -1, means the new-designed part will not be used
    break;
	     ...
	     FILE *stream)		/* output stream */
{
  switch (pred->class) {
  case BPredComb:
    bpred_dir_config (pred->dirpred.bimod, "new_mode", stream);           //print new-mode    
		...
		struct stat_sdb_t *sdb)	/* stats database */
{
  ...
  if (pred->class == BPredComb)
    {
      sprintf(buf, "%s.used_new_mode", name);
      stat_reg_counter(sdb, buf, 
		       "total number of new_mode predictions used", 
  ...
  switch (pred_dir->class) {
	...
	int l1index, l2index;
        ...
        if (pred_dir->config.two.xor)
	  {
	    ...
	    l2index = (((l2index ^ (baddr >> MD_BR_SHIFT))                            //GShare algorithm
					 ...
					 * used on mispredict recovery */
{
  ...
  switch (pred->class) {
      ...
      if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND))
	{
	  ...
	  twolev = bpred_dir_lookup (pred->dirpred.twolev, baddr);             //meta-predictor is deleted

	  dir_update_ptr->dir.bimod = (*bimod >= 2);
	  dir_update_ptr->dir.twolev  = (*twolev >= 2);

	      dir_update_ptr->pdir1 = twolev;                        //any case, the main predictor is two-level predictor
	      dir_update_ptr->pdir2 = bimod;                        //the second predictor is new-mode predictor
  ...
  if (pbtb == NULL)
    {
    	...
    	if(*(dir_update_ptr->pdir2)==-1)                        //if new-mode counter is -1, new predictor is not used
    {
	...
	else                                                         //new-mode predictor is used, combine two counter together
    {    
	   int a=*dir_update_ptr->pdir1<<2|*dir_update_ptr->pdir2;            //4-bit counter result
       return ((a >= 8)                                             //if counter >= 1000 =8 taken
	      ? /* taken */ 1
	      : /* not taken */ 0);    
    }
  }
	...
	else         /* BTB hit, so return target if it's a predicted-taken branch */
    {
	     ...
	     struct bpred_update_t *dir_update_ptr)/* pred state pointer */
{
  ...
  if (dir_update_ptr->dir.ras)
	...
	pred->used_2lev++;
      if(dir_update_ptr->pdir1 && *dir_update_ptr->pdir2!=-1)
	pred->used_bimod++;
    }
  ...
  if (dir_update_ptr->pdir1 && *dir_update_ptr->pdir2==-1)                //if we only use 2-level predictor to predict
    {
      if (taken)                                                         //prediction is correct
	{
	  if (*dir_update_ptr->pdir1 < 3)                                  //if counter less than 3, counter +1
	    ++*dir_update_ptr->pdir1;
	}
      else
	{ /* not taken */                                                     //prediciton is wrong
	  if (*dir_update_ptr->pdir1 > 0)
	    --*dir_update_ptr->pdir1;                                      //if counter larger than 0, counter -1
	}
    }
    
      if (dir_update_ptr->pdir1 && *dir_update_ptr->pdir2!=-1)                 //we use both predictor
    {int a = *dir_update_ptr->pdir1 <<2 | *dir_update_ptr->pdir2;              //4-bit counter
      if (taken)                                                              //prediction is correct
	{
	  if (a <15)                                                            //if counter less than 15, counter +1
	   a++;
	}
      else
	{ /* not taken */                                                         //prediction is wrong
	  if (a> 0)
	    a--;                                                                 //if counter larger than 0, counter -1
	}
	*dir_update_ptr->pdir1=a>>2;                                     //update counter stored in PHT to the first two bit of the new 4-bit counter
	*dir_update_ptr->pdir2=a^ ((a>>2)<<2);                            //update counter stored in PHT to the last two bit of the new 4-bit counter 
    }
